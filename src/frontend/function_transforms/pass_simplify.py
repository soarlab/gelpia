...
def pow_replacement(exp, inputs, assigns, consts):
  def expand(exp):
    mul_list = []
    def int_pow(exp):
      if exp[0] == "pow":
    ...
    def _expand(exp):
      if exp[0] != "*":
        ...
        mul_list.extend(int_pow(exp))
      else:
        _pow_replacement(exp[2])
        mul_list.extend(int_pow(exp[2]))
        _expand(exp[1])
    _expand(exp)
    mul_list.sort()
    return mul_list
  ...
  def compress(mul_list):
    ...
    while exp in mul_list:
      mul_list.remove(exp)
    if expo > 1:
      return ["pow", exp, ["Integer", str(expo)]]
    return exp
  ...
  def _pow_replacement(exp, from_mul=False):
    ...
    if exp[0] in {"pow"} and int(exp[2][1]) == 1:
    ...
    if exp[0] in {"Variable"}:
      _pow_replacement(assigns[exp[1]])
      return
