...
then
    ...
    wget https://static.rust-lang.org/dist/rust-1.37.0-x86_64-unknown-linux-gnu.tar.gz -O rust_nightly.tar.gz >> $SCRIPT_LOCATION/log.txt 2>&1
    mkdir -p rust_nightly && tar -xf rust_nightly.tar.gz -C rust_nightly --strip-components 1
    cd rust_nightly
    ./install.sh --prefix=$SCRIPT_LOCATION >> $SCRIPT_LOCATION/log.txt 2>&1
fi
...
./configure --enable-sse2 --prefix=$SCRIPT_LOCATION >> $SCRIPT_LOCATION/log.txt 2>&1
make >> log.txt 2>&1
...
cd $SOURCE_LOCATION
rm -f gaol.tar.gz
wget http://downloads.sourceforge.net/project/gaol/gaol/4.2.0/gaol-4.2.0.tar.gz -O gaol.tar.gz >> $SCRIPT_LOCATION/log.txt 2>&1
tar xf gaol.tar.gz
patch -p0 < $SCRIPT_LOCATION/../documents/gaol-4.2.0.patch >> $SCRIPT_LOCATION/log.txt 2>&1
mv gaol-4.2.0 gaol
cd gaol
...
./configure  --with-mathlib=crlibm --enable-simd --enable-preserve-rounding=yes\
	     ...
	     --prefix=$SCRIPT_LOCATION >> $SCRIPT_LOCATION/log.txt 2>&1
make >> $SCRIPT_LOCATION/log.txt 2>&1
make install >> $SCRIPT_LOCATION/log.txt 2>&1
